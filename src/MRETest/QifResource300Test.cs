...
namespace MRETest
{
    /// <summary> Tests QIF 3.0 loading/saving. </summary>
    [TestClass]
    public class QifResource300Test : BaseTest
    {
        /// <summary> Loads and saves a QIF 3.0 file and compares the result with etalon result. </summary>
        ...
        {
            string input = Path.Combine(TestFilePath, "QIF.3.0.0", fileName);
            string result = GenerateResultFileName(input);

            // load
            var docInput = new QifResourcesDocument(input);
            var cmms = docInput.Load();

            // save
            var docResult = new QifResourcesDocument(result) { Qpid = docInput.Qpid };
            docResult.Save(cmms);

            // examine the resulting file
            Assert.IsTrue(ExamineFile(result, GenerateEtalonFileName(input)));
        }
    }
}
