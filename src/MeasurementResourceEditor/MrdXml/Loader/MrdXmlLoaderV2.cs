...
{
    /// <summary> Loader of MRD documents of version 2. </summary>
    internal class MrdXmlLoaderV2 : AMrdXmlLoader
    {
        #region Constructor

        /// <summary> Constructor. </summary>
        public MrdXmlLoaderV2(XmlDocument doc)
            : base(doc)
        {
        }
        #endregion
        ...
        /// <summary> Loads an MRD file of version 2. </summary>
        ...
        public override List<CmmParameters> Load(XmlElement rootElement)
        {
            ...
            var cmmNodes = rootElement.SelectNodes("Cmm");
            foreach (XmlNode cmmXmlNode in cmmNodes)
            {
                // Load the CMM node and add it to the list
                loadedCmms.Add(LoadCmmXmlNode(cmmXmlNode));
            }
        ...
        private CmmParameters LoadCmmXmlNode(XmlNode node)
        {
            ...
            // CMM Type
            cmmParameters.Type = ToCmmType(GetSingleValue(node, "Type"));
            cmmParameters.AxisOrientationX = ToCmmAxisOrientation(GetSingleValue(node, "AxisOrientationX"));
            cmmParameters.AxisOrientationY = ToCmmAxisOrientation(GetSingleValue(node, "AxisOrientationY"));
            cmmParameters.AxisOrientationZ = ToCmmAxisOrientation(GetSingleValue(node, "AxisOrientationZ"));
        ...
        /// <param name="cmmParameters"> CMM parameter to fill </param>
        ...
        /// <param name="node"> XML node </param>
        /// <returns> ISO 10360 performance test </returns>
        ...
        private SensorParameters LoadSensorXmlNode(XmlNode node)
        {
            ...
            // Get probe orientation stuff
            if (sensorParameters.Type == SensorType.SingleTipArticulated)
            {
                sensorParameters.BAngleAdjustment = GetSingleDValue(node, "BAngleAdjustment");
            }
            else
            {
                sensorParameters.AAngle = GetSingleDValue(node, "AAngle");
                sensorParameters.BAngle = GetSingleDValue(node, "BAngle");
            }
        ...
        /// <returns> True if vector is valid, otherwise false </returns>
        ...
        /// <summary> Converts the given string to a CmmSoftwareCompensationType enumeration type. </summary>
        /// <param name="value"> String </param>
        /// <returns> Enumeration </returns>
        ...
        /// <summary> Converts the given string to a Cmm.CmmType enumeration. </summary> 
        /// <param name="value"> String </param>
        /// <returns> Enumeration </returns>
        CmmType ToCmmType(string value)
        {
            value = value.ToLower();
            switch (value)
            {
                case "movingbridge":
                    return CmmType.MovingBridge;
                case "fixedbridge":
                    return CmmType.FixedBridge;
                case "column":
                    return CmmType.Column;
                case "fixedtablecantilever":
                    return CmmType.FixedTableCantilever;
                case "movingramhorizontalarm":
                    return CmmType.MovingRamHorizontalArm;
                case "movingtablehorizontalarm":
                    return CmmType.MovingTableHorizontalArm;
                case "gantry":
                    return CmmType.Gantry;
                case "lshapedbridge":
                    return CmmType.LShapedBridge;
                case "fixedtablehorizontalarm":
                    return CmmType.FixedTableHorizontalArm;
                case "movingtablecantileverarm":
                    return CmmType.MovingTableCantileverArm;
            }
            throw new ArgumentException("Unknown performance test type: " + value);
        }

        /// <summary> Converts the given string to a CmmAxisOrientation (not case-sensitive). </summary>
        /// <param name="value"> String </param>
        /// <returns> Enumeration </returns>
        CmmAxisOrientation ToCmmAxisOrientation(string value)
        {
            foreach (var v in Enum.GetValues(typeof(CmmAxisOrientation)))
            {
                string enumName = Enum.GetName(typeof(CmmAxisOrientation), v);
                if (string.Equals(enumName, value, StringComparison.OrdinalIgnoreCase))
                    return (CmmAxisOrientation)v;
            }
