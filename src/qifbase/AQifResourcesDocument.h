...
#include "Data/CmmParameters.h"
#include <vector>
#include <memory>

namespace qifbase
{
/// <summary> Interface for QIF resources document. </summary>
class QIFBASE_EXPORT AQifResourcesDocument
{
public:
    AQifResourcesDocument() = default;
    virtual ~AQifResourcesDocument() = default;

    /// <summary> Saves the given CMM list to the QIF document. Returns true if success. </summary>
    /// <param name="cmmParameters"> CMMs to save </param>
    /// <returns> True in case of success, otherwise false </returns>
    virtual bool Save(const std::vector<std::unique_ptr<CmmParameters> >& cmmParameters) = 0;

    /// <summary> Loads a list of CMMs from the QIF document. Returns true if success.  </summary>
    /// <param name="cmmList"> Loaded CMMs </param>
    /// <returns> True in case of success, otherwise false </returns>
    virtual bool Load(std::vector<std::unique_ptr<CmmParameters> >& cmmList) = 0;

    // Error and Warning lists
    virtual const std::vector<std::wstring>& ErrorList() = 0;
    virtual const std::vector<std::wstring>& ErrorDetailsList() = 0;
    virtual const std::vector<std::wstring>& WarningList() = 0;
    virtual const std::vector<std::wstring>& WarningDetailsList() = 0;

    // Returns true if there was an error/warning
    virtual bool HadError() = 0;
    virtual bool HadWarning() = 0;
};
}
